...
import ScrollDownHint from "../invitations/ScrollDownHint";
import {
  BOY_THEMES,
  GIRL_THEMES,
...
export const meta: TemplateMeta = {
  ...
  name: 'Castel Regal',
  category: 'wedding',
  description: 'Design de poveste cu castel regal si efect de deschidere orizontala pe baza de scroll.',
...
const mergeByThemeId = (themes: CastleColorTheme[]): CastleColorTheme[] => {
  const seen = new Set<string>();
  const output: CastleColorTheme[] = [];
...
export const GIRL_CASTLE_SIMPLE_THEMES: CastleColorTheme[] = mergeByThemeId([
  ...(GIRL_THEMES as unknown as CastleColorTheme[]),
  ...(BOY_THEMES as unknown as CastleColorTheme[]),
]);

const getGirlCastleSimpleTheme = (id?: string): CastleColorTheme =>
  GIRL_CASTLE_SIMPLE_THEMES.find((theme) => theme.id === id) ||
  GIRL_CASTLE_SIMPLE_THEMES[0];
    ...
    <div style={{ position: 'absolute', top: inviteTopPos, left: 0, right: 0, transform: inviteTransform, textAlign: 'center', zIndex: 1, padding: '0 36px', pointerEvents: editMode ? 'auto' : 'none' }}>
      <div ref={inviteRef} style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '100%', opacity: inviteOpacity }}>
        <InlineEdit tag="p" editMode={!!editMode} value={inviteTop || 'Cu bucurie va anuntam'} onChange={v => onInviteTopChange?.(v)}
          ...
          style={{ fontFamily: 'Cinzel, serif', fontSize: '0.72rem', fontWeight: 800, letterSpacing: '0.44em', textTransform: 'uppercase', color: tc.pinkXL, textShadow: '0 3px 16px rgba(0,0,0,0.95), 0 0 10px rgba(255,255,255,0.2)', margin: 0 }} />
        ...
        <InlineEdit tag="p" editMode={!!editMode} value={inviteBottom || 'a fost botezat'} onChange={v => onInviteBottomChange?.(v)}
          ...
          style={{ fontFamily: 'Cinzel, serif', fontSize: '0.74rem', fontWeight: 600, letterSpacing: '0.28em', textTransform: 'uppercase', color: tc.pinkXL, textShadow: '0 3px 14px rgba(0,0,0,0.92), 0 0 8px rgba(255,255,255,0.18)', margin: 0, lineHeight: 1.85 }} />
        <InlineEdit tag="p" editMode={!!editMode} value={inviteTag || ' deschide portile '} onChange={v => onInviteTagChange?.(v)}
          ...
          style={{ fontFamily: 'Cinzel, serif', fontSize: '0.58rem', fontWeight: 700, letterSpacing: '0.5em', textTransform: 'uppercase', color: `${tc.pinkXL}dd`, margin: '2px 0 0', textShadow: '0 2px 10px rgba(0,0,0,0.82)' }} />
      </div>
    </div>
  </div>
  );
...
const CastleIntro: React.FC<{
  ...
  themeColors?: { pinkDark: string; pinkL: string; pinkXL: string; gold: string };
        ...
        previewMode = 'doors', embeddedPreview = false, childName = '', subtitle = 'in my castle', welcomeText = 'WELCOME',
        inviteTop, inviteMiddle, inviteBottom, inviteTag, dateStr,
        onChildNameChange, onSubtitleChange, onWelcomeChange,
        onInviteTopChange, onInviteMiddleChange, onInviteBottomChange, onInviteTagChange,
  ...
  if (previewMode === 'static') {
      ...
      return (
        ...
        >
          ...
          <CastleOverlayText
            ...
            inviteTop={inviteTop}
            inviteMiddle={inviteMiddle}
            inviteBottom={inviteBottom}
            inviteTag={inviteTag}
            dateStr={dateStr}
            onInviteTopChange={onInviteTopChange}
            onInviteMiddleChange={onInviteMiddleChange}
            onInviteBottomChange={onInviteBottomChange}
    ...
    return (
      <div
        style={{
          position: 'relative',
          height: embeddedPreview ? '100%' : 800,
          borderRadius: 12,
          marginBottom: embeddedPreview ? 0 : 32,
          overflow: 'hidden',
        }}
      >
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.08)' }} />
        <CastleOverlayText
          childName={childName} subtitle={subtitle} welcomeText={welcomeText} editMode={!!editMode}
  ...
  return (
    ...
    >
      <div ref={leftDoorRef} style={{ position: 'absolute', top: 0, left: 0, width: '50%', height: embeddedPreview ? '100%' : '100dvh', overflow: 'visible', willChange: 'transform' }}>
      ...
      <div ref={rightDoorRef} style={{ position: 'absolute', top: 0, right: 0, width: '50%', height: embeddedPreview ? '100%' : '100dvh', overflow: 'visible', willChange: 'transform' }}>
...
export const SIMPLE_GIRL_CASTLE_BLOCKS: InvitationBlock[] = getSharedDefaultBlocks("castle-magic-girl-simple");
...
export const CASTLE_PREVIEW_DATA = {
  ...
  project: { selectedTemplate: 'castle-magic-girl-simple' },
  profile: {
    ...CASTLE_DEFAULTS,
    colorTheme: 'default',
...
const CastleMagicTemplateGirl: React.FC<InvitationTemplateProps & {
...
}> = ({ data, onOpenRSVP, editMode = false, introPreview = false, introOnly = false, suppressAudioModal = false, enableGsapPreview = false, scrollContainer, onProfileUpdate }) => {
  ...
  // In simple flow we stay isolated for layout/blocks, but intro door images
  // should still come from dashboard API config if available.
  const [templateImageConfig, setTemplateImageConfig] = useState<Record<string, any>>({});
  useEffect(() => {
    let cancelled = false;

    const loadTemplateImageConfig = async () => {
      const candidateIds = [meta.id, "castle-magic-girl"];
      ...
      for (const templateId of candidateIds) {
        try {
          ...
          const hasImages =
            !!cfg?.heroBgImage ||
            !!cfg?.heroBgImageMobile ||
            (cfg?.themeImages && Object.keys(cfg.themeImages || {}).length > 0);

          if (hasImages) {
            if (!cancelled) setTemplateImageConfig(cfg || {});
            return;
          }
        } catch {
          // try next id
      ...
      if (!cancelled) setTemplateImageConfig({});
    };

    loadTemplateImageConfig();
    return () => {
      cancelled = true;
    };
  }, []);
  ...
  const validGirlThemeIds = new Set(
    GIRL_CASTLE_SIMPLE_THEMES.map((theme) => theme.id),
  );
  const userThemeRaw = (pr as any).colorTheme;
  const apiThemeRaw = templateImageConfig?.colorTheme;
  const activeColorTheme = validGirlThemeIds.has(userThemeRaw)
    ? userThemeRaw
    : validGirlThemeIds.has(apiThemeRaw)
      ? apiThemeRaw
      : "default";
  const theme = getGirlCastleSimpleTheme(activeColorTheme);
  ...
  const normalizeBlocks = (input: InvitationBlock[]): InvitationBlock[] =>
    input.map((block, idx) => ({
      ...block,
      id: block.id || `simple-girl-${idx}`,
    }));

  const cloneDefaultBlocks = (): InvitationBlock[] =>
    normalizeBlocks(SIMPLE_GIRL_CASTLE_BLOCKS);

  // Blocurile invitatiei se citesc din profile.customSections cand exista.
  // Daca lipsesc, folosim blocurile default ale template-ului simplu.
  const [blocks, setBlocks] = useState<InvitationBlock[]>(() => {
    const fromProfile = safeJSON(profile.customSections, null);
    if (Array.isArray(fromProfile) && fromProfile.length > 0) {
      return normalizeBlocks(fromProfile as InvitationBlock[]);
    }
    return cloneDefaultBlocks();
  });
  const lastSyncedBlocksRef = useRef<string>("");

  // Sync inbound: daca profilul se actualizeaza din exterior, il reflectam in preview.
  useEffect(() => {
    const fromProfile = safeJSON(profile.customSections, null);
    ...
    const nextBlocks =
      fromProfile.length > 0
        ? normalizeBlocks(fromProfile as InvitationBlock[])
        : cloneDefaultBlocks();
    const serialized = JSON.stringify(nextBlocks);

    if (serialized === lastSyncedBlocksRef.current) return;
    lastSyncedBlocksRef.current = serialized;
    setBlocks(nextBlocks);
  }, [profile.customSections]);

  // Sync outbound: orice editare de carduri se salveaza in profile.customSections.
  useEffect(() => {
    const serialized = JSON.stringify(blocks);
    if (serialized === lastSyncedBlocksRef.current) return;
    lastSyncedBlocksRef.current = serialized;
    onProfileUpdate?.({ customSections: serialized });
  }, [blocks, onProfileUpdate]);

  // Imagini usi  din admin per tema activa
  const apiThemeImages = (
    templateImageConfig?.themeImages &&
    typeof templateImageConfig.themeImages === "object"
      ? templateImageConfig.themeImages
      : {}
  ) as Record<string, { desktop?: string; mobile?: string }>;
  const apiThemeEntry =
    apiThemeImages[activeColorTheme] || apiThemeImages.default || {};
  const heroBgImage =
    apiThemeEntry.desktop ||
    templateImageConfig?.heroBgImage;
  const heroBgImageMobile =
    apiThemeEntry.mobile ||
    templateImageConfig?.heroBgImageMobile;
  ...
  const useStaticIntroPreview = !editMode && suppressAudioModal && !enableGsapPreview;
  ...
  useEffect(() => {
    if (!editMode && !suppressAudioModal) {
      setShowAudioModal(hasMusicBlock());
      return;
    }
    setShowAudioModal(false);
  }, [editMode, hasMusicBlock, suppressAudioModal]);
  useEffect(() => {
  ...
  const resetToDefaults = useCallback(() => {
    ...
    const freshBlocks = cloneDefaultBlocks();
  ...
  const addBlock = useCallback((type: string, def: any) => {
    setBlocks((prev) => [
      ...prev,
      { id: Date.now().toString(), type: type as InvitationBlockType, show: true, ...def },
    ]);
  }, []);
  ...
  if (introOnly) {
    // Keep intro dimensions aligned with Boy template in wizard previews.
    const isEmbeddedIntroPreview = editMode;
    return (
      <>
        ...
        >
          <div
            className={cn(
              "max-w-4xl mx-auto",
              isEmbeddedIntroPreview
                ? "h-full px-2 md:px-4 py-0"
                : "px-0 md:px-6 pt-2 md:pt-5",
            )}
          >
            <BlockStyleProvider value={{ blockId: "__intro__" } as any}>
              <CastleIntro
                editMode={editMode}
                previewMode="static"
                embeddedPreview={isEmbeddedIntroPreview}
                ...
                onChildNameChange={
                  editMode ? (v) => upProfile("partner1Name", v) : undefined
                }
                onSubtitleChange={
                  editMode ? (v) => upProfile("castleIntroSubtitle", v) : undefined
                }
                onWelcomeChange={
                  editMode ? (v) => upProfile("castleIntroWelcome", v) : undefined
                }
                onInviteTopChange={
                  editMode ? (v) => upProfile("castleInviteTop", v) : undefined
                }
                onInviteMiddleChange={
                  editMode ? (v) => upProfile("castleInviteMiddle", v) : undefined
                }
                onInviteBottomChange={
                  editMode ? (v) => upProfile("castleInviteBottom", v) : undefined
                }
                onInviteTagChange={
                  editMode ? (v) => upProfile("castleInviteTag", v) : undefined
                }
              />
            </BlockStyleProvider>
          </div>
  ...
  return (
    <>
      ...
      {showAudioModal && !editMode && !suppressAudioModal && (
      ...
      {showIntro && (
        <BlockStyleProvider value={{ blockId: "__intro__" } as any}>
          <CastleIntro
            editMode={useStaticIntroPreview ? false : editMode}
            previewMode={useStaticIntroPreview ? "static" : "doors"}
            embeddedPreview={!editMode && suppressAudioModal}
            onDone={() => {}}
            castleUrl={heroBgImage}
            castleUrlMobile={heroBgImageMobile}
            contentEl={contentEl}
              ...
              .map((block, idx) => (
                <div key={block.id} className="group/insert">
                  ...
                  >
                    <BlockStyleProvider
                      value={{
                        blockId: block.type === "timeline" ? "__timeline__" : block.id,
                      } as any}
                    >
                      {editMode && (
                        <BlockToolbar
