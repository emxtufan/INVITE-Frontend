...
import { getMermaidTheme } from "../invitations/castleDefaults";
...
const JurassicDoorIntro: React.FC<{
  ...
  onDone, editMode, previewMode = 'doors', embeddedPreview = false, contentEl, scrollContainer,
...
export const CASTLE_DEFAULT_BLOCKS: InvitationBlock[] = getSharedDefaultBlocks("little-mermaid-simple");
...
const LittleMermaidTemplate: React.FC<InvitationTemplateProps & {
  editMode?: boolean; introPreview?: boolean; introOnly?: boolean;
  ...
  const shouldUseIntro                       = !editMode && !introPreview && !introOnly;
  ...
  useEffect(()=>{setShowIntro(!editMode && !introPreview && !introOnly);},[editMode,introPreview,introOnly]);
  ...
  if (introOnly) {
    return (
      <>
        ...
        <div
          style={{
            background: `linear-gradient(180deg,${C.darkJungle} 0%,${hexToRgba(C.midJungle,.92)} 40%,${C.darkJungle} 100%)`,
            backgroundColor: C.darkJungle,
            fontFamily: sans,
            minHeight: editMode ? "100%" : "100vh",
            padding: editMode ? "0" : "8px 0",
          }}
        >
          <div style={{ maxWidth: 896, margin: "0 auto", padding: editMode ? "0 8px" : "0 12px" }}>
            <div style={{ height: "min(70vh, 760px)", minHeight: 520 }}>
              <BlockStyleProvider value={{ blockId: "__intro__", textStyles: introTextStyles } as any}>
                <JurassicDoorIntro
                  editMode={editMode}
                  previewMode="static"
                  embeddedPreview
                  childName={p.partner1Name || "Ariel"}
                  partner2Name={p.partner2Name}
                  isWedding={isWedding}
                  subtitle={p.castleIntroSubtitle}
                  inviteTop={p.castleInviteTop}
                  inviteMiddle={p.castleInviteMiddle || dateStr}
                  inviteBottom={p.castleInviteBottom}
                  inviteTag={p.castleInviteTag}
                  dateStr={dateStr}
                  doorImg={heroBgImage}
                  doorImgMobile={heroBgImageMobile}
  ...
  return (
    <>
      ...
      {/* Audio modal */}
      {showAudioModal && !editMode && !introPreview && (
