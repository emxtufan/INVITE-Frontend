...
import { getGabbyTheme } from "../invitations/castleDefaults";
...
const Countdown:React.FC<{targetDate:string|undefined}>=({targetDate})=>{
  ...
  if(isOver) return(
    <div style={{textAlign:'center',padding:'12px 20px',
      ...
      <p style={{fontFamily:F.label,fontSize:12,color:C.deepPink,margin:0}}>{"\u{1F389} Petrecerea a inceput! \u{1F389}"}</p>
    </div>
  );
    ...
    <div>
      <div style={{display:'flex',justifyContent:'center',marginBottom:12}}>
        <span style={{fontFamily:F.label,fontSize:9,letterSpacing:'.4em',textTransform:'uppercase',
          ...
          {isSoon ? '\u{1F380} Maine!' : '\u{23F0} Timp ramas'}
        </span>
      </div>
...
export const CASTLE_DEFAULT_BLOCKS: InvitationBlock[] = getSharedDefaultBlocks("gabbys-dollhouse-simple");
...
const GabbysDollhouseTemplate: React.FC<InvitationTemplateProps & {
  ...
  useEffect(() => {
    if (editMode || isEmbeddedPreview) {
      setShowIntro(false);
      setContentVisible(true);
    }
  }, [editMode, isEmbeddedPreview]);
              ...
              <div style={{...card,textAlign:'center',padding:'0 0 28px'}}>
                ...
                <Reveal from="bottom" delay={0.45}>
                  <div style={{padding:'14px 20px',
                    ...
                    />
                    <div style={{position:'absolute',top:-10,left:'50%',transform:'translateX(-50%)',fontSize:20}}>{"\u{1F451}"}</div>
